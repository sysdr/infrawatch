  ...
  return (
    <div className="dashboard">
      <div className="dashboard-header">
        ...
        <p>Real-time metrics - start.sh runs demo automatically for non-zero data</p>
      </div>
      ...
      <Row gutter={[16, 16]} className="table-row">
        <Col span={24}>
          <Card title="Recent Metrics">
            <Table columns={columns} dataSource={chartData} rowKey={(r) => `${r.timestamp || ''}-${r.cpu ?? ''}-${r.memory ?? ''}-${r.disk ?? ''}`} pagination={{ pageSize: 10 }} loading={loading} />
          </Card>
        </Col>
      </Row>
    </div>
